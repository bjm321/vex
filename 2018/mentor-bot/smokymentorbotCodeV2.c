...
void sidemovement(int time, int speed);
...
void moveBot(int speed, int targetDistance)
{
	int power, buffer = 500, error = 10;
	power = (speed / 4) * 3;

	clearMotorEncoder();

	while (nMotorEncoder[frontRight] < targetDistance - buffer)
	{
		if (nMotorEncoder[frontRight] == nMotorEncoder[frontLeft]) // If rightEncoder has counted the same amount as leftEncoder:
		{
			// Move Forward
		...
		else if (nMotorEncoder[frontRight] > nMotorEncoder[frontLeft])	// If rightEncoder has counted more encoder counts
		{
			// Turn slightly right
			motor[frontRight] = power;
			motor[backRight] = power;
	...
	while (nMotorEncoder[frontRight] > targetDistance + error || nMotorEncoder[frontRight] < targetDistance - error)
	{

		if (nMotorEncoder[frontRight] > targetDistance)
		{
			if (nMotorEncoder[frontRight] == nMotorEncoder[frontLeft]) // If rightEncoder has counted the same amount as leftEncoder:
			{
				// Move Forward
